...
app.post('/admin/orders/:id/assign', (req, res) => {
  ...
  if (deliveryBoy === 'all' || deliveryBoy === '') {
    // Assign to ALL available riders (first come first serve)
    // Set rider_id to 'all' - all riders will see it
    db.prepare('UPDATE orders SET delivery_boy = ?, rider_id = ? WHERE id = ?').run('all', 'all', id);
  } else {
...
app.get('/admin/dashboard', (req, res) => {
  ...
  try {
    ...
    // Pending tickets (safe - table may not exist)
    let pendingTickets = 0;
    try { pendingTickets = db.prepare(`SELECT COUNT(*) as count FROM support_tickets WHERE status = 'open'`).get()?.count || 0; } catch(e) {}
    
    // Unread chats (safe)
    let unreadChats = 0;
    try { unreadChats = db.prepare(`SELECT COUNT(DISTINCT phone) as count FROM chat_messages WHERE sender = 'user' AND created_at >= datetime('now', '-1 day')`).get()?.count || 0; } catch(e) {}

    res.json({ ok: true, counts, today, total, recent, topProducts, pendingTickets, unreadChats });
  } catch(e) {
...
// DB setup for rider
try {
  db.exec(`CREATE TABLE IF NOT EXISTS rider_earnings (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS rider_cod (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    rider_id TEXT NOT NULL,
    order_id INTEGER NOT NULL,
  ...
  // Add rider columns to orders if not exist
  try { db.prepare('ALTER TABLE orders ADD COLUMN rider_id TEXT').run(); } catch(e) {}
  try { db.prepare('ALTER TABLE orders ADD COLUMN delivery_otp TEXT').run(); } catch(e) {}
...
app.post('/api/rider/status', (req, res) => {
  ...
  if (idx === -1) return res.status(404).json({ ok: false, error: 'rider not found' });
...
app.get('/api/rider/orders/:riderId', (req, res) => {
  ...
  if (status === 'new') {
    ...
    orders = db.prepare(`SELECT * FROM orders WHERE (rider_id = ? OR delivery_boy = ? OR rider_id = 'all') ORDER BY id DESC LIMIT 50`).all(riderId, riderId);
  }
...
app.post('/api/rider/order/accept', (req, res) => {
  ...
  if (order.rider_id !== 'all' && order.rider_id !== riderId && order.status === 'out_for_delivery') {
    return res.status(400).json({ ok: false, error: 'Order already accepted by another rider' });
  }
  
  // Lock order to this rider + generate OTP + set status to preparing (rider will pick up)
  const otp = String(Math.floor(1000 + Math.random() * 9000));
  db.prepare(`UPDATE orders SET status = 'preparing', rider_id = ?, delivery_boy = ?, rider_accepted_at = datetime('now'), delivery_otp = ? WHERE id = ?`).run(riderId, riderId, otp, orderId);
  
  // Notify customer
  const updatedOrder = db.prepare('SELECT * FROM orders WHERE id = ?').get(orderId);
  if (updatedOrder) setImmediate(() => notifyCustomer(updatedOrder, 'preparing'));
...
app.post('/api/rider/order/status', (req, res) => {
  ...
  // Notify customer
  const updatedOrder = db.prepare('SELECT * FROM orders WHERE id = ?').get(orderId);
  if (updatedOrder) setImmediate(() => notifyCustomer(updatedOrder, status));
...
app.post('/api/rider/order/verify', (req, res) => {
  ...
  // Credit earnings (prevent duplicate)
  const existingEarning = db.prepare('SELECT id FROM rider_earnings WHERE rider_id = ? AND order_id = ?').get(riderId, orderId);
  if (!existingEarning) {
    const settings = require('./data/settings').get();
    const zones = settings.deliveryZones || [];
  ...
  res.json({ ok: true, message: 'Delivery verified! Earnings credited.' });
});
...
app.get('/api/rider/earnings/:riderId', (req, res) => {
  ...
  const recent = db.prepare(`SELECT * FROM rider_earnings WHERE rider_id = ? ORDER BY created_at DESC LIMIT 20`).all(riderId);
